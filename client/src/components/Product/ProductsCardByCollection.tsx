...
export function ProductsCardByCollection() {
  ...
  const getCollections = async () => {
    try {
      ...
      if (res) {
        setCollections(res);
        res.forEach((col: Collection) => getProductsByCollection(col.id));
      }
    } catch (e) {
  ...
  const SkeletonRow = () => (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="animate-pulse bg-base-200 h-72 rounded-xl" />
      ))}
    </div>
  );
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {items.length > 0 ? `${items.length} sản phẩm` : "Đang cập nhật sản phẩm"}
                  </p>
                </div>
                {items.length > 0 && (
                  ...
                  <>
                    {items.map((item) => (
                      <Link
                        to={`/products/${item.id}`}
                        key={item.id}
                        className="flex-none w-64 snap-start transition-transform hover:-translate-y-1"
                      >
                        <div className="card w-64 bg-white shadow-md border rounded-xl h-full">
                          <figure className="relative w-full h-52 bg-white rounded-t-xl p-4 overflow-hidden">
                            <img
                              src={item.imageUrl}
                              alt={item.name}
                          ...
                          <div className="card-body">
                            ...
                            <button
                              ...
                              className="btn text-white mt-2 border verify-supp-color bg-white hover:bg-gray-50"
                            >
                              Add to Cart
                            </button>
                          </div>
                    ...
                    {/* More card */}
                    <Link to={`/collections/${col.id}`} className="flex-none w-64 snap-start">
                      <div className="card w-64 h-full bg-gradient-to-br from-gray-50 to-gray-200 shadow-inner flex items-center justify-center rounded-xl border border-gray-200 hover:shadow-lg transition">
